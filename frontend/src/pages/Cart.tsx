...
export function Cart() {
  ...
  const shippingEst = subtotal > 10000 ? "견적 후 확정" : "$480 (예상)";
  const validIds = items.filter(i => i.approved).map(i => i.id);
    ...
    <div className="max-w-[1280px] mx-auto px-4 py-8 font-[Inter,sans-serif]">
      ...
      {items.length === 0 ? (
        <div className="text-center py-20">
          ...
          <Link to="/suppliers" className="bg-primary hover:bg-primary/90 text-white px-8 py-3 rounded font-semibold text-sm transition-colors inline-block">
            제품 탐색하기
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-[1fr_340px] gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-white border border-border rounded p-3 flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-foreground">전체 선택 ({selected.length}/{validIds.length})</span>
            </div>
            {items.map((item) => {
              const moqShortfall = item.quantity < item.moq;
              const isSelected = selected.includes(item.id);
              return (
                <div key={item.id} className={`bg-white border rounded overflow-hidden ${moqShortfall ? "border-amber-300" : "border-border"}`}>
                  <div className="flex items-start gap-4 p-5">
                    <button
                      ...
                      className={`w-5 h-5 border-2 rounded flex items-center justify-center flex-shrink-0 mt-7 transition-colors ${
                        !item.approved ? "border-muted bg-muted cursor-not-allowed" :
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-6 mt-3">
                        ...
                        <div className="text-xs text-muted-foreground font-mono">최소 주문: {item.moq.toLocaleString()}개</div>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <div className="font-bold text-foreground font-mono">
                          {item.currency}{(item.price * item.quantity).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                        </div>
                      </div>
          ...
          <div className="space-y-4">
            <div className="bg-white border border-border rounded p-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between text-muted-foreground">
                  <span>관세 (예상)</span>
                  <span className="font-mono text-green-600">FTA 적용 시 0%</span>
                </div>
              ...
              <Link to="/suppliers" className="mt-2 w-full border border-border text-muted-foreground hover:border-primary hover:text-primary py-2.5 rounded text-sm font-medium transition-colors flex items-center justify-center gap-2 block text-center">
                쇼핑 계속하기
              </Link>
            </div>
