...
import { ArrowLeft, ArrowRight, Eye, EyeOff, Upload, User, Building2, MapPin, AlertCircle } from "lucide-react";
...
interface FormData {
    ...
    managerName: string;
    managerPhone: string;
    delegationDoc: File | null;     // 직원 전용: 위임장
    employmentDoc: File | null;     // 직원 전용: 재직증명서
    // Step 2: 사업자 정보
    businessNo: string;
    companyName: string;
    ceoName: string;
            ...
            <Field label="비밀번호" required>
                <div className="relative">
                    <input type={showPw ? "text" : "password"} value={form.password}
                           ...
                           placeholder="8자 이상" className={`${inputCls} pr-10`} />
            ...
            <Field label="비밀번호 확인" required>
                <div className="relative">
                    <input type={showConfirm ? "text" : "password"} value={form.confirmPassword}
                           ...
                           className={`${inputCls} pr-10 ${pwMismatch ? "border-red-400 focus:border-red-400" : ""}`} />
...
function Step2({ form, set }: { form: FormData; set: (f: Partial<FormData>) => void }) {
    ...
    const handleLookup = () => {
        ...
        setLoading(true);
        setTimeout(() => {
            set({ companyName: "(주)패션코리아", ceoName: "홍길동" });
            setLoading(false);
            setFetched(true);
        }, 800);
    };
...
function Step3({ form, set }: { form: FormData; set: (f: Partial<FormData>) => void }) {
    return (
        <div className="space-y-4">
            <div className="flex items-center gap-1.5 text-sm font-semibold text-foreground">
                <MapPin size={14} className="text-primary" /> 기본 배송지
            </div>

            <p className="text-xs text-muted-foreground bg-muted/40 rounded p-3 -mt-1">
                사업장 주소와 다를 수 있으므로 실제 상품을 수령할 주소를 입력해 주세요.
            </p>
            ...
            <div className="bg-muted/40 border border-border rounded p-4 space-y-1.5">
                ...
                {[
                    ...
                    ["배송지",         form.addressMain ? `${form.addressMain} ${form.addressDetail}`.trim() : "–"],
                ].map(([k, v]) => (
