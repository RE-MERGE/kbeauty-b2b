...
import { Heart, ShoppingCart, Search, Filter, Grid3x3, List, ChevronDown } from "lucide-react";
...
const searchDummyCategories = [
  ...
  { id: "sports", name: "스포츠/애슬레저", iconImg: "/images/sports.png", alias: ["스포츠", "운동복", "수영복", "요가복", "필라테스"], subCategories: ["스포츠탑", "스포츠레깅스", "트레이닝복", "스포츠세트"] },
  { id: "accessories", name: "액세서리", iconImg: "/images/accessory.png", alias: ["악세서리", "악세사리"], subCategories: ["가방/백", "모자", "스카프/머플러", "벨트", "양말/타이즈"] },
...
export function Wishlist() {
  ...
  const wishlistProducts = products.filter((p) => {
    ...
    const matchSearch = searchTab !== "product" || !searchQuery ||
      p.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      p.brand.toLowerCase().includes(searchQuery.toLowerCase());
    return isFav && matchCategory && matchSub && matchBrand && matchSearch;
  });
  ...
  return (
    <div className="max-w-[1480px] mx-auto px-4 py-8 font-[Inter,sans-serif]">
          ...
          <div className="flex border border-border rounded-lg">
            ...
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={
                  searchTab === "product" ? "좋아요 한 상품명으로 검색" :
                  searchTab === "category" ? "좋아요 한 카테고리명으로 검색" : "좋아요 한 브랜드명으로 검색"
                }
                value={searchQuery}
          ...
          {resultDropOpen && searchQuery.trim().length > 0 && (
            <div ref={resultDropRef} className="absolute left-0 right-0 mt-1 bg-white border border-border rounded shadow-xl z-[100] max-h-72 overflow-y-auto">
              {searchResults.length === 0 ? (
                ...
                (searchResults as typeof products).map((p) => (
                  <Link
                    key={p.id}
                    to={`/products/${p.id}`}
                  ...
                  >
                    ...
                    <div className="text-primary text-sm font-bold flex-shrink-0">₩{p.price.toLocaleString()}</div>
                  </Link>
                ))
              ) : searchTab === "category" ? (
                (searchResults as typeof searchDummyCategories).map((c) => (
                  <button
                    key={c.id}
                    onClick={() => { handleCategoryChange(c.id); setResultDropOpen(false); setSearchQuery(""); }}
      ...
      <div className="grid grid-cols-[240px_1fr] gap-6">
        {/* [수정] Sidebar - AllProducts와 동일한 구조 */}
