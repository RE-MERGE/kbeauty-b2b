...
export function SellerOrders() {
  ...
  const handleStartPreparing = (orderId: number) => {
    ...
    setNotice("출고 준비 상태로 변경했습니다. 현재는 화면 확인용 처리입니다.");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <main className="mx-auto w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8">
        ...
        {notice && (
          <div className="mb-4 flex items-center justify-between gap-3 border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">
            <span>{notice}</span>
            <button
              type="button"
              onClick={() => setNotice("")}
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1050px] table-fixed text-left">
              <thead className="bg-slate-50 text-xs font-bold text-slate-500">
                <tr>
                  <th className="w-[17%] px-5 py-3">주문</th>
                  <th className="w-[25%] px-4 py-3">담당 상품</th>
                  <th className="w-[11%] px-4 py-3">상태</th>
                  <th className="w-[13%] px-4 py-3">주문일</th>
                  <th className="w-[12%] px-4 py-3">출고 기한</th>
                  <th className="w-[11%] px-4 py-3 text-right">상품 금액</th>
                  <th className="w-[11%] px-5 py-3 text-right">처리</th>
                </tr>
              </thead>
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-right">
                        {order.status === "PENDING" ? (
                          <span className="inline-flex h-9 items-center gap-1.5 bg-slate-100 px-3 text-xs font-bold text-slate-500">
                            <Clock3 size={13} />
                            결제 대기 중
                          </span>
                          ...
                          <Link
                            to={`/seller/orders/${order.orderId}`}
                            className="inline-flex h-9 items-center gap-1.5 border border-slate-200 px-3 text-xs font-bold text-slate-600 transition hover:border-primary hover:text-primary"
                          >
                            상세 보기
                            <ArrowRight size={13} />
                          </Link>
