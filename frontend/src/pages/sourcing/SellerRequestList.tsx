...
// ── 출고 모달 (운송장 입력) ───────────────────────────────────────────
// ── 소싱 요청 상세 모달 (셀러용) ─────────────────────────────────────
function SellerSourcingDetailModal({
                                     req, onClose,
                                   }: {
  req: ReadyRequest | CustomRequest;
  onClose: () => void;
}) {
  const navigate = useNavigate();
  const isCustom = "totalBudget" in req;
  const custom = isCustom ? (req as CustomRequest) : null;
  const ready = !isCustom ? (req as ReadyRequest) : null;
        ...
        <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-[520px] max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
            <div>
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                ...
                {isCustom ? `${req.category} > ${(req as CustomRequest).subCategory}` : req.category}
              </span>
              </div>
              <h3 className="font-bold text-foreground">{req.productName}</h3>
              <div className="text-xs text-muted-foreground mt-0.5">{req.buyerName} · 바이어</div>
            </div>
          ...
          <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <div className={`grid divide-x divide-border border border-border rounded-lg overflow-hidden ${isCustom ? "grid-cols-4" : "grid-cols-3"}`}>
              {[
                { label: "희망 수량", value: `${req.quantity.toLocaleString()}벌`, icon: <Package size={12} /> },
                {
                  label: isCustom ? "전체 예산" : "희망 단가",
                  value: isCustom
                      ? `${(custom!.totalBudget / 10000).toLocaleString()}만원`
                      : `${ready!.unitPrice.toLocaleString()}원`,
                  icon: <CreditCard size={12} />,
                },
                { label: "희망 납기", value: req.deliveryDate, icon: <Calendar size={12} /> },
                ...(isCustom ? [{
                  label: "샘플",
                  value: custom!.needSample === "Y" ? "필요" : "불필요",
                  icon: <FlaskConical size={12} />,
                  highlight: custom!.needSample === "Y",
                }] : []),
            ...
            {/* 작업지시서 (CUSTOM) */}
            {isCustom && custom!.workFileCount > 0 && (
                <div>
                  <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                    작업지시서 ({custom!.workFileCount}건)
                  </div>
                  <div className="space-y-1.5">
                    {Array.from({ length: custom!.workFileCount }).map((_, i) => (
                        <div key={i} className="flex items-center gap-3 border border-border rounded px-3 py-2 bg-secondary hover:border-primary transition-colors cursor-pointer group/file">
                          ...
                          <span className="text-xs text-foreground flex-1">작업지시서_{req.id}_{i + 1}.pdf</span>
            ...
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 text-sm text-blue-700">
              <BadgeCheck size={14} />
              현재 <strong className="mx-1">{req.bidCount}개</strong> 공급사가 견적을 제출했습니다.
            </div>
          </div>
          ...
          <div className="px-6 py-4 border-t border-border flex gap-2 flex-shrink-0">
            ...
            <button
                onClick={() => {
                  onClose();
                  navigate(`/sourcing/${req.id}/quote`, { state: { request: req } });
                }}
...
function CustomRow({
                     req, myBids, onAddBid, onViewBid, onShip, onDetail,
                   }: {
  req: CustomRequest;
  myBids: MyBid[];
      ...
      <div className="bg-white border border-border rounded-lg overflow-hidden hover:border-primary/50 transition-colors">
        ...
        <div className="px-5 py-4 flex items-center gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-muted-foreground mt-0.5 cursor-pointer" onClick={() => onDetail(req)}>{req.buyerName}</div>
          </div>
...
// ── READY 카드 (기존 유지) ────────────────────────────────────────────
function ReadyCard({ req, myBids, onBid, onDetail }: { req: ReadyRequest; myBids: MyBid[]; onBid: (r: ReadyRequest) => void; onDetail: (r: ReadyRequest) => void }) {
  const dday = getDday(req.deliveryDate);
  ...
  return (
      <div className="bg-white border border-border rounded-lg overflow-hidden hover:border-primary hover:shadow-md transition-all group">
        ...
        <div className="p-5">
          ...
          <h3 className="font-bold text-foreground text-sm leading-tight group-hover:text-primary transition-colors mb-0.5 cursor-pointer" onClick={() => onDetail(req)}>{req.productName}</h3>
          <div className="text-xs text-muted-foreground mb-3 cursor-pointer" onClick={() => onDetail(req)}>{req.buyerName}</div>
          <div className="grid grid-cols-3 gap-2 mb-4 cursor-pointer" onClick={() => onDetail(req)}>
...
export function SellerRequestList() {
  ...
  const [selectedDetail, setSelectedDetail] = useState<ReadyRequest | CustomRequest | null>(null);
          ...
          {activeTab === "READY" && subTab !== "past" && (
              <>
                <div className="grid grid-cols-3 gap-4">
                  {readyList.map((req) => (
                      <ReadyCard key={req.id} req={req} myBids={myBids[req.id] ?? []} onBid={setSelectedReadyReq} onDetail={setSelectedDetail} />
                  ))}
                </div>
                {readyList.length === 0 && (
